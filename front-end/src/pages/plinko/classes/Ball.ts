...
//import ballPng from "../../../assets/ball.svg"; // tutaj podmienisz ścieżkę na swoją

/*
const ballImg = new Image();
ballImg.src = ballPng;

ballImg.onload = () => console.log("Ball sprite loaded");
*/
...
export class Ball {
  ...
  public isFinished(): boolean {
    ...
    console.log(this.targetSinkIndex);
  }
  ...
  draw() {
   ...
   /*
    ...
    const cy = unpad(this.y);
    const VISUAL_SCALE = 1.5; // <- dopasuj (1.6–2.2 idealne)
    const size = this.radius * 2 * VISUAL_SCALE;
    if (this.sprite && this.sprite.complete && this.sprite.naturalWidth > 0) {
      this.ctx.save();
      this.ctx.drawImage(this.sprite, cx - this.radius, cy - this.radius, size, size);
      this.ctx.restore();
      return;
    }
